...
public interface ProductRepo extends JpaRepository<Product, Integer> {
    @Query(value = "select * from product where quantity > 0 limit 9", nativeQuery = true)
    ...
    @Query(nativeQuery = true, value = "select * from product where categorytypeid =1 and quantity >0 limit 9")
    public List<Product> dressCategory();

    @Query(nativeQuery = true, value = "select * from product where categorytypeid =2 and quantity >0 limit 9")
    public List<Product> panCategory();

    @Query(nativeQuery = true, value = "select * from product where categorytypeid =3 and quantity >0 limit 9")
    public List<Product> shirtCategory();

    @Query(nativeQuery = true, value = "select * from product where productdetail like %:searchInput% and quantity >0 and productstatusid =1")
    public List<Product> searchProduct(@Param("searchInput") String searchInput);

    @Query(nativeQuery = true, value = "select * from product where productdetail like %:searchInput%  and quantity > 0 and productstatusid =1 limit 5")
